...
public class SshAuthenticationService extends Service {
    ...
    private Intent checkIntent(Intent intent) {
        ...
        if (errorResult == null) {
            ...
            Timber.e("Check requirements failed");
            return errorResult;
        }
    }
    ...
    private Intent authenticate(Intent data) {
        ...
        if (authResult.isPending()) {
            ...
            // Sanitize error messages for external callers - don't expose internal details
            // Reuse callingPackage from earlier in the method
            String errorMessage;
            if (callingPackage != null && callingPackage.equals(getPackageName())) {
                // Internal call - provide detailed error
                errorMessage = getString(errorMsg.mType.getMsgId());
            } else {
                // External call - provide generic error
                errorMessage = "Authentication failed";
            }
            return createErrorResult(SshAuthenticationApiError.INTERNAL_ERROR, errorMessage);
        }
        } catch (Exception e) {
            Timber.e(e, "Exception in authenticate");
            // Always return generic error to external callers
            return createErrorResult(SshAuthenticationApiError.INTERNAL_ERROR, "Authentication error");
        }
    }
